...
pub fn cobble<P: Into<PathBuf>>(path: P, prefix: Option<String>) -> impl IntoIterator<Item = Line> {
    ...
    fn process_file(name: &OsStr, entry: DirEntry, prefix: Option<String>) -> Line {
        let path = entry
            .path()
            .iter()
            .skip(1)
    ...
    fn process_entry(name: &OsStr, entry: DirEntry, prefix: Option<String>) -> Option<Vec<Line>> {
...
pub fn generate_cobweb_manifest<P: Into<PathBuf>>(manifest_path: P) -> String {
